...
if(grid){
    const initMasonry = ()=>{
        ...
        const bannerFix = ()=>{
            const elems = msnry.getItemElements();
            elems.forEach(el=>{
    ...
    const stopMasonry = ()=>{
        msnry?.element ? msnry.destroy() : null;
        grid.getElementsByClassName('case-banner').forEach((el)=>{
            el.classList.remove('case-banner_right')
        })
    }
    
    window.addEventListener("resize", ()=>{ 
        setTimeout(()=>{window.innerWidth > 992 ? initMasonry() : stopMasonry()}, 200)
    })
...
for(let trigger of triggers){
    trigger.onclick = (e)=>{
        ...
        const targetClasses = trigger.getAttribute("targets").split(' ');
...
//input file
const inputsFile = document.querySelectorAll("[type='file']");
for (let inputFile of inputsFile) {
    inputFile.addEventListener('change', (e)=>{
        console.log(e.target.files);
        let inputDiv = e.target.parentNode;
        let fileList = inputDiv.querySelectorAll(".input-file__filelist")[0];
        fileList.innerHTML = "";
        e.target.files.forEach((file)=>{
            let fileNodeClone = inputDiv.querySelectorAll(".input-file__example .input-file__file")[0].cloneNode(true);
...
if(document.querySelectorAll(".head-index__scrolldown")[0]){
    document.querySelectorAll(".head-index__scrolldown")[0].onclick = (e)=>{
        e.preventDefault(); 
        document.querySelectorAll(".index-info")[0].scrollIntoView({block: "center", behavior: "smooth"});
    };
}
//scrollto end